...
export function useWeatherByDate(date: string) {
  ...
  if (data && (data as any).weatherByDate && Array.isArray((data as any).weatherByDate)) {
    // TODO FIX DATA TYPING
    legacyData = (data as any).weatherByDate.map((item: any) =>
      convertToLegacyFormat(item as WeatherData)
    ); // TODO FIX DATA TYPING
  }
